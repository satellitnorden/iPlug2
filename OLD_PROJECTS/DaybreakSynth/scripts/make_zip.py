...
def  main():
  ...
  installer = "\installer\DaybreakSynth Installer.exe"
   
  if demo:
    installer = "\installer\DaybreakSynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DaybreakSynth manual.pdf" 
  ]
  ...
  ZIPNAME = "DaybreakSynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DaybreakSynth-v" + FULLVERSIONSTR + "-win-demo.zip"
