...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=DaybreakLimiter Demo Installer\n"
      else:
        line="OutputBaseFilename=DaybreakLimiter Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the DaybreakLimiter Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the DaybreakLimiter installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=DaybreakLimiter Demo installer\n"
     else:
       line="SetupWindowTitle=DaybreakLimiter installer\n"
