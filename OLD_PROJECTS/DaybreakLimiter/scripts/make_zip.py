...
def  main():
  ...
  installer = "\installer\DaybreakLimiter Installer.exe"
   
  if demo:
    installer = "\installer\DaybreakLimiter Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DaybreakLimiter manual.pdf" 
  ]
  ...
  ZIPNAME = "DaybreakLimiter-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DaybreakLimiter-v" + FULLVERSIONSTR + "-win-demo.zip"
