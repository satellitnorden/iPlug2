...
def  main():
  ...
  installer = "\installer\MIDIGate Installer.exe"
   
  if demo:
    installer = "\installer\MIDIGate Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MIDIGate manual.pdf" 
  ]
  ...
  ZIPNAME = "MIDIGate-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MIDIGate-v" + FULLVERSIONSTR + "-win-demo.zip"
