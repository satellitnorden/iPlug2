...
def  main():
  ...
  installer = "\installer\AlesisDrums Installer.exe"
   
  if demo:
    installer = "\installer\AlesisDrums Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\AlesisDrums manual.pdf" 
  ]
  ...
  ZIPNAME = "AlesisDrums-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "AlesisDrums-v" + FULLVERSIONSTR + "-win-demo.zip"
