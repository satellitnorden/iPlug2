...
def  main():
  ...
  installer = "\installer\DaybreakGain Installer.exe"
   
  if demo:
    installer = "\installer\DaybreakGain Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DaybreakGain manual.pdf" 
  ]
  ...
  ZIPNAME = "DaybreakGain-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DaybreakGain-v" + FULLVERSIONSTR + "-win-demo.zip"
