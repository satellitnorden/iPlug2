...
def  main():
  ...
  installer = "\installer\AutoGainStager Installer.exe"
   
  if demo:
    installer = "\installer\AutoGainStager Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\AutoGainStager manual.pdf" 
  ]
  ...
  ZIPNAME = "AutoGainStager-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "AutoGainStager-v" + FULLVERSIONSTR + "-win-demo.zip"
