...
def  main():
  ...
  installer = "\installer\DrumMIDIGenerator Installer.exe"
   
  if demo:
    installer = "\installer\DrumMIDIGenerator Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DrumMIDIGenerator manual.pdf" 
  ]
  ...
  ZIPNAME = "DrumMIDIGenerator-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DrumMIDIGenerator-v" + FULLVERSIONSTR + "-win-demo.zip"
