...
def  main():
  ...
  installer = "\installer\InfinityBass Installer.exe"
   
  if demo:
    installer = "\installer\InfinityBass Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\InfinityBass manual.pdf" 
  ]
  ...
  ZIPNAME = "InfinityBass-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "InfinityBass-v" + FULLVERSIONSTR + "-win-demo.zip"
