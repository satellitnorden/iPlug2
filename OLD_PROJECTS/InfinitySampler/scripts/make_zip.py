...
def  main():
  ...
  installer = "\installer\InfinitySampler Installer.exe"
   
  if demo:
    installer = "\installer\InfinitySampler Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\InfinitySampler manual.pdf" 
  ]
  ...
  ZIPNAME = "InfinitySampler-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "InfinitySampler-v" + FULLVERSIONSTR + "-win-demo.zip"
