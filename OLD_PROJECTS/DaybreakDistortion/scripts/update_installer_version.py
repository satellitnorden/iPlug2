...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=DaybreakDistortion Demo Installer\n"
      else:
        line="OutputBaseFilename=DaybreakDistortion Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the DaybreakDistortion Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the DaybreakDistortion installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=DaybreakDistortion Demo installer\n"
     else:
       line="SetupWindowTitle=DaybreakDistortion installer\n"
