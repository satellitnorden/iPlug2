...
def  main():
  ...
  installer = "\installer\DaybreakDistortion Installer.exe"
   
  if demo:
    installer = "\installer\DaybreakDistortion Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DaybreakDistortion manual.pdf" 
  ]
  ...
  ZIPNAME = "DaybreakDistortion-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DaybreakDistortion-v" + FULLVERSIONSTR + "-win-demo.zip"
