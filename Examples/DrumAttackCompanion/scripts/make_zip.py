...
def  main():
  ...
  installer = "\installer\DrumAttackCompanion Installer.exe"
   
  if demo:
    installer = "\installer\DrumAttackCompanion Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DrumAttackCompanion manual.pdf" 
  ]
  ...
  ZIPNAME = "DrumAttackCompanion-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DrumAttackCompanion-v" + FULLVERSIONSTR + "-win-demo.zip"
