...
def  main():
  ...
  installer = "\installer\InfinityDrums Installer.exe"
   
  if demo:
    installer = "\installer\InfinityDrums Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\InfinityDrums manual.pdf" 
  ]
  ...
  ZIPNAME = "InfinityDrums-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "InfinityDrums-v" + FULLVERSIONSTR + "-win-demo.zip"
