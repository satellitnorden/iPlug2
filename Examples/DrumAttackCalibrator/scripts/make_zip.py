...
def  main():
  ...
  installer = "\installer\DrumAttackCalibrator Installer.exe"
   
  if demo:
    installer = "\installer\DrumAttackCalibrator Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DrumAttackCalibrator manual.pdf" 
  ]
  ...
  ZIPNAME = "DrumAttackCalibrator-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DrumAttackCalibrator-v" + FULLVERSIONSTR + "-win-demo.zip"
