...
def  main():
  ...
  installer = "\installer\InfinitySynth Installer.exe"
   
  if demo:
    installer = "\installer\InfinitySynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\InfinitySynth manual.pdf" 
  ]
  ...
  ZIPNAME = "InfinitySynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "InfinitySynth-v" + FULLVERSIONSTR + "-win-demo.zip"
