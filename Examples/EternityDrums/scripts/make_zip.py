...
def  main():
  ...
  installer = "\installer\EternityDrums Installer.exe"
   
  if demo:
    installer = "\installer\EternityDrums Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\EternityDrums manual.pdf" 
  ]
  ...
  ZIPNAME = "EternityDrums-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "EternityDrums-v" + FULLVERSIONSTR + "-win-demo.zip"
