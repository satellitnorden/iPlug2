...
def  main():
  ...
  installer = "\installer\DrumEngine Installer.exe"
   
  if demo:
    installer = "\installer\DrumEngine Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DrumEngine manual.pdf" 
  ]
  ...
  ZIPNAME = "DrumEngine-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DrumEngine-v" + FULLVERSIONSTR + "-win-demo.zip"
