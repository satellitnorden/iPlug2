...
def  main():
  ...
  installer = "\installer\LiveDrumRemapper Installer.exe"
   
  if demo:
    installer = "\installer\LiveDrumRemapper Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\LiveDrumRemapper manual.pdf" 
  ]
  ...
  ZIPNAME = "LiveDrumRemapper-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "LiveDrumRemapper-v" + FULLVERSIONSTR + "-win-demo.zip"
