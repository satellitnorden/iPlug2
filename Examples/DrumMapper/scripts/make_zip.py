...
def  main():
  ...
  installer = "\installer\DrumMapper Installer.exe"
   
  if demo:
    installer = "\installer\DrumMapper Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DrumMapper manual.pdf" 
  ]
  ...
  ZIPNAME = "DrumMapper-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DrumMapper-v" + FULLVERSIONSTR + "-win-demo.zip"
