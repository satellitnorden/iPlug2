...
def  main():
  ...
  installer = "\installer\DaybreakPanInvert Installer.exe"
   
  if demo:
    installer = "\installer\DaybreakPanInvert Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DaybreakPanInvert manual.pdf" 
  ]
  ...
  ZIPNAME = "DaybreakPanInvert-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DaybreakPanInvert-v" + FULLVERSIONSTR + "-win-demo.zip"
