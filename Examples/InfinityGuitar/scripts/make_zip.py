...
def  main():
  ...
  installer = "\installer\InfinityGuitar Installer.exe"
   
  if demo:
    installer = "\installer\InfinityGuitar Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\InfinityGuitar manual.pdf" 
  ]
  ...
  ZIPNAME = "InfinityGuitar-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "InfinityGuitar-v" + FULLVERSIONSTR + "-win-demo.zip"
