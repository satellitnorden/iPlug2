...
InfinitySampler::InfinitySampler(const InstanceInfo& info)
  : Plugin(info, MakeConfig(NUMBER_OF_PARAMS, 1))
{
  ...
  mMakeGraphicsFunc = [&]() { return MakeGraphics(*this, PLUG_WIDTH, PLUG_HEIGHT, PLUG_FPS, 1.0f); };
  ...
  mLayoutFunc = [&](iplug::igraphics::IGraphics* pGraphics) {
    ...
    {
      iplug::igraphics::IVButtonControl* const file_button{new iplug::igraphics::IVButtonControl(
        bounds.GetCentredInside(100).GetVShifted(-100).GetHShifted(-100), [this](iplug::igraphics::IControl* control) { SelectFile(); }, "FILE")};
    ...
    {
      iplug::igraphics::IVButtonControl* const folder_button{new iplug::igraphics::IVButtonControl(
        bounds.GetCentredInside(100).GetVShifted(-100).GetHShifted(100), [this](iplug::igraphics::IControl* control) { SelectFolder(); }, "FOLDER")};
...
void InfinitySampler::OnReset()
{
  ...
  {
    playing_note._SoundResourcePlayer.GetADSREnvelope(0).SetSampleRate(GetSampleRate());
    playing_note._SoundResourcePlayer.GetADSREnvelope(1).SetSampleRate(GetSampleRate());
  }
}
...
void InfinitySampler::ProcessBlock(iplug::sample** inputs, iplug::sample** outputs, const int32 number_of_frames)
{
  ...
  for (int32 i{0}; i < number_of_frames; ++i)
  {
      ...
      while (!_MidiQueue.Empty())
      {
        ...
        if (message.mData1 == GetParam(MIDI_NOTE)->Int())
        {
          ...
          if (status == iplug::IMidiMsg::kNoteOn)
          {
            ...
            playing_note._SoundResourcePlayer.SetGain(
              gain + CatalystRandomMath::RandomFloatInRange(-InfinitySamplerConstants::MAXIMUM_HUMANIZE_GAIN, InfinitySamplerConstants::MAXIMUM_HUMANIZE_GAIN) * _HumanizeFactor);
            playing_note._SoundResourcePlayer.GetADSREnvelope(0).SetSampleRate(GetSampleRate());
            playing_note._SoundResourcePlayer.GetADSREnvelope(1).SetSampleRate(GetSampleRate());
            playing_note._SoundResourcePlayer.GetADSREnvelope(0).SetStageValues(
              GetParam(ATTACK_VALUE)->Value(), GetParam(DECAY_VALUE)->Value(), GetParam(SUSTAIN_VALUE)->Value(), GetParam(RELEASE_VALUE)->Value());
            playing_note._SoundResourcePlayer.GetADSREnvelope(1).SetStageValues(
              GetParam(ATTACK_VALUE)->Value(), GetParam(DECAY_VALUE)->Value(), GetParam(SUSTAIN_VALUE)->Value(), GetParam(RELEASE_VALUE)->Value());
            playing_note._SoundResourcePlayer.GetADSREnvelope(0).EnterAttackStage();
            playing_note._SoundResourcePlayer.GetADSREnvelope(1).EnterAttackStage();
          }
        }
      ...
      for (PlayingNote& playing_note : _PlayingNotes)
      {
        playing_note._SoundResourcePlayer.Advance(0);
        playing_note._SoundResourcePlayer.Advance(1);
      }
  }
}

void InfinitySampler::ProcessMidiMsg(const iplug::IMidiMsg& msg) { _MidiQueue.Add(msg); }
#endif
 ...
 */
void CatalystPlatform::PrintToOutput(const char* const RESTRICT message) NOEXCEPT { OutputDebugString(message); }
#endif